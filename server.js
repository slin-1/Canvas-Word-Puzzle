...
/*
Use a browser to view the program: http://localhost:3000/index.html

The program allows the user to type a puzzle file name into the textbox, that when valid, loads the 
words onto the canvas in scrambled order.

The user can then attempt to "solve" the puzzle by dragging the words into the correct order.
Note: As long as the words are in the correct order, it will register as correct (so the words do not have
      to be in the same line as the text file)
*/

//Ctrl+C to stop server (in command line terminal)
        ...
        if (exists) {
          ...
          fs.readFile(puzzleFile, function(err, data) {
            ...
            if (err) {
              ...
              returnObj.validity = "VALID"
              returnObj.puzzleWords = fileLines //obtain each puzzle word, but it shouldn't show the answer's order to the user
